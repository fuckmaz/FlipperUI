...
pub async fn rfid_scan(
    ...
    let operation = state.operations.begin(OperationName::RfidScan)?;
...
#[tauri::command]
pub fn rfid_cancel_scan(operation_id: u64, state: State<AppState>) -> Result<()> {
    require_cancelled(
        state
            .operations
            .cancel(OperationName::RfidScan, operation_id),
    )
}
